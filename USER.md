# USER.md - About My Human

- **Name:** Josh
- **What to call them:** Josh
- **Pronouns:** _(not specified)_
- **Timezone:** UTC _(current session context; confirm later if needed)_
- **Notes:** Building toward a location-independent, family-centered, worldschool lifestyle supported by semi-passive digital income.

## Context

Josh was a 5th grade teacher for about 10 years.
His wife taught anthropology and history at the high school and college levels.
Josh currently works in real estate with his wife, mainly wholesaling, with some fix-and-flips and rentals.

He wants to transition away from real estate and build a more meaningful, location-independent life for his family.

Current goals and themes:
- build automated/semi-passive income streams targeting $20K+ per month
- create digital products, tools, systems, or communities
- focus on families, homeschoolers, curiosity-driven learning, personal growth, and AI-enabled leverage
- design a business ecosystem that supports worldschooling and travel
- turn existing education and curiosity-driven expertise into digital assets and products
- emphasize polymath-style, curiosity-led learning rather than traditional achievement-driven education
- use AI to support learning as exploration, synthesis, and direct experience
- avoid building primarily for the traditional school system

Existing asset:
- Josh recently published an ebook: Curiosity-Driven Classroom Using AI (hosted on SamCart)
- The ebook has not yet been marketed and has not produced sales yet

Underused strengths:
- pattern synthesis across disciplines
- teaching and guiding curiosity
- storytelling around meaning and growth
- visionary thinking
- AI curiosity and experimentation

Communication preferences:
- one-on-one
- brief and conversational

Dislikes / friction:
- admin work, paperwork, bookkeeping, data entry, logistics
- technical setup and troubleshooting
- cold outreach, aggressive sales, transactional lead chasing
- repetitive manual work
- work disconnected from meaning or purpose

Important constraints:
- avoid time-for-money models as the main path
- avoid high-pressure sales
- avoid unnecessary complexity and research spirals
- avoid projects with constant manual upkeep unless they lead to automation or scalable assets
- focus on a small number of high-leverage opportunities
- prioritize long-term assets over short-term cash grabs
- avoid work tied to a fixed location or rigid schedule
- automate or delegate what AI can reasonably handle
